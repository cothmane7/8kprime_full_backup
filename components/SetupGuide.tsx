                                ...
                                <div className="p-8 md:p-12">
                                    ...
                                    <div className="space-y-8 mb-10">
                                        {activeContent?.steps.map((step, index) => {
                                            if (activeDevice === 'windows' && index === 1) return null;
                                            if (activeDevice === 'smart-tv' && index === 2) return null;
                                    ...
                                    {activeDevice === 'windows' && activeContent && activeContent.apps.length > 0 && (
                                        <div className="mb-10 pl-14">
                                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                                {activeContent.apps.map((app, index) => (
                                                    <div key={index} className="bg-[#121217] border border-white/5 rounded-2xl p-6 flex flex-col items-center text-center hover:border-primary/20 transition-all group">
                                                        <div className="text-[50px] mb-4 drop-shadow-xl filter grayscale group-hover:grayscale-0 transition-all duration-300">{app.icon}</div>
                                                        <h4 className="text-white font-black text-xs mb-5 uppercase tracking-widest">{app.name}</h4>
                                                        <a href={app.url} className="w-full bg-primary hover:bg-accent-bronze text-black font-black py-4 px-4 rounded-xl flex items-center justify-center gap-2 transition-all text-[10px] uppercase tracking-widest shadow-lg shadow-primary/20">
                                                            <Download size={14} />
                                                            {app.store}
                                                        </a>
                                                    </div>
                                    ...
                                    {activeDevice === 'smart-tv' && activeContent && activeContent.apps.length > 0 && (
                                        <div className="mb-10 pl-14">
                                            <div className="flex flex-wrap gap-4">
                                                {activeContent.apps.map((app, index) => (
                                                    <div key={index} className="w-[180px] bg-[#121217] border border-white/5 rounded-2xl p-6 flex flex-col items-center text-center hover:border-primary/20 transition-all">
                                                        <div className="text-[50px] mb-4">{app.icon}</div>
                                                        <h4 className="text-white font-black text-sm mb-4 uppercase">{app.name}</h4>
                                                        <button className="w-full bg-[#2A2A35] text-gray-300 font-bold py-2.5 px-4 rounded-xl flex items-center justify-center gap-2 transition-colors text-[11px] uppercase tracking-wider">
                                                            {app.store}
                                                        </button>
                                                    </div>
                                                ))}
                                    ...
                                    {activeDevice === 'smart-tv' && activeContent && activeContent.steps[2] && (
                                        <div className="flex gap-6 mb-10 font-medium">
                                            ...
                                            <div className="text-gray-300 leading-relaxed pt-[6px] text-[15px]">
                                                {activeContent.steps[2].text}
                                            </div>
                                        </div>
                                    )}

                                    {activeDevice === 'windows' && activeContent && activeContent.steps[1] && (
                                        <div className="flex gap-6 mb-10 font-medium">
                                            <div className="w-8 h-8 rounded-full bg-[#2A2000] border border-[#B08D3E]/30 flex items-center justify-center text-[#B08D3E] font-bold shrink-0 mt-1 shadow-[0_0_15px_rgba(176,141,62,0.1)] text-sm">
                                                2
                                            </div>
                                            <div className="text-gray-300 leading-relaxed pt-[6px] text-[15px]">
                                                {activeContent.steps[1].text}
                                            </div>
                                        </div>
                                    )}

                                    {activeDevice !== 'windows' && activeDevice !== 'smart-tv' && activeContent && activeContent.apps.length > 0 && (
                                        <div className="pl-14 mb-10">
                                            {activeDevice === 'firestick' ? (
                                            ...
                                            ) : activeDevice === 'apple' || activeDevice === 'macos' ? (
                                                <div className="flex flex-wrap gap-4">
                                                    {activeContent.apps.map((app, index) => (
                                                        <div key={index} className="w-[180px] bg-[#121217] border border-white/5 rounded-2xl p-6 flex flex-col items-center text-center hover:border-primary/20 transition-all group">
                                                            <div className="text-[50px] mb-4">{app.icon}</div>
                                                            <h4 className="text-white font-black text-sm mb-5 uppercase">{app.name}</h4>
                                                            <button className="w-full bg-[#8394A8] hover:bg-[#6b7b8d] text-white font-bold py-2.5 px-4 rounded-xl flex items-center justify-center gap-2 transition-colors text-xs">
                                                                🍎
                                                                App Store
                                                            </button>
                                                        </div>
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                                                    {activeContent.apps.map((app, index) => (
                                                        <div key={index} className="bg-[#121217] border border-white/5 rounded-2xl p-6 flex flex-col items-center text-center hover:border-primary/20 transition-colors group">
                                                            ...
                                                            {!app.codeOnly && (
                                                            <button className={`w-full font-black py-4 px-4 rounded-xl flex items-center justify-center gap-2 transition-all text-[10px] uppercase tracking-widest ${app.active
                                    ...
                                    <div className="bg-primary/5 border border-primary/20 rounded-[2rem] p-8 flex items-start gap-6 mb-8 group hover:bg-primary/10 transition-colors">
                                    ...
                                    {activeDevice === 'smart-tv' && (
                                        <div className="flex flex-wrap gap-3 mt-8">
                                            {activeContent?.brands?.map((brand, i) => (
                                                <div key={i} className={`flex flex-col items-center justify-center border rounded-xl py-3 px-2 w-[110px] sm:w-[120px] transition-all
                                                    ${brand === '& All Others'
